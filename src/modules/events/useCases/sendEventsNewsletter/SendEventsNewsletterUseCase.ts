...
async function formatValues(events: Event[]): Promise<ValuesFormatted[]> {
  const result = await Promise.all(
    events.map(async (event): Promise<ValuesFormatted> => {
      ...
      let startDateFormatted = new Intl.DateTimeFormat("pt-BR", {
        weekday: "long",
        day: "2-digit",
        month: "long",
        hour: "numeric",
      ...
      startDateFormatted =
        startDateFormatted.charAt(0).toUpperCase() +
        startDateFormatted.slice(1);
...
class SendEventsNewsletterUseCase {
  ...
  async execute(date: Date, isTest = false, fakeData?: any): Promise<void> {
    ...
    const hasPromotion = await this.promotionsRepository.findByDate(
      `${this.dateProvider.getYear(date)}-${String(
        this.dateProvider.getMonth(date) + 1
      ).padStart(2, "0")}-${String(this.dateProvider.getDay(date)).padStart(
        2,
        "0"
      )}`
    );

    const events = await this.eventsRepository.findByHighlightAndWillStart(
      year,
      month,
      day
    );
